...
logger = logging.getLogger('discord_scheduler')
...
def run_bot():
    ...
    try:
        ...
        result = subprocess.run(
            ...
            text=True,
            timeout=60,  # 타임아웃 증가
            cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # 프로젝트 루트로 설정
        )
...
def wait_until_next_scheduled_time():
    ...
    logger.info(f"[SCHEDULE] 다음 실행 예정 시간: {next_run.strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    logger.info(f"[TIMEZONE] 현재 시스템 시간: {current_time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    ...
    if current_minute % 10 == 0:
        ...
        logger.info(f"[WAIT] 현재 시간: {current_minute}분 - 다음 10분 단위까지 대기합니다")
